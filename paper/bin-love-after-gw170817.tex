...
The unconstrained set is then defined by drawing random samples in the spectral coefficients (within their allowed prior ranges), and then eliminating any EoS that either leads to a polytropic index $\Gamma$ outside the allowed range, breaks the causality restriction, or breaks the maximum mass restriction. The constrained set is defined in a similar way, with the addition of the restriction that the resulting EoS be inside the 90\% marginalized posterior distribution on the pressure-mass density plane derived from the GW170817 observation~\cite{LIGO:posterior,Carney:2018sdv}  (see Fig.~2 in~\cite{LIGO:posterior}). In both cases, each set consists of 100 members, with a subset of these shown in the left panel of Fig.~\ref{fig:eos}.\kc{wait, aren't you using the samples that Calr sent you directly? If yes we need to describe how they were computed and add the appropriate citations to code and data. But that's not what you are describing here}
...
The fractional difference from the fits, shown in the bottom panels, is greatly suppressed for the constrained case, compared to both the unconstrained case, and results from previous works~\cite{Yagi:ILQ}.
...
Square root of of the spectral noise densities $\sqrt{S_n^A(f)}$ plotted for LIGO O2, aLIGO at design sensitivity, A\texttt{+}, Voyager, CE, and ET-D as interpolated from publicly available data.
