...
\end{bmatrix},
\end{align}
{\zc{I'll Decide whether to use 3x3 or 3x2 coefficient matrices and change this (or not) later.}}
...
Given sampled parameters $\Lambda_s$ and $q$, $\Lambda_a^{\text{relation}}(\Lambda_s,q)$ corresponds to the value found from the binary Love fit found in Sec.~\ref{sec:binary}, and $\Lambda_a^{\text{true}}$ corresponds to the true value predicted by the various sampled EoSs for the same values of $q$ and $\Lambda_s$
...
Such an approximation would allow us to compare the systematic and statistical uncertainties in order to determine when the former will dominate the latter.
...
However, future detectors (for example aLIGO at design sensitivity, A\texttt{+}, Voyager, CE, and ET) are planned with large reductions in sensitivity - both decreasing the statistical uncertainties and allowing for larger binary NS merger detection rates; further reducing the statistical uncertainties.
