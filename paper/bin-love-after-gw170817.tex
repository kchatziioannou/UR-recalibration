...
\begin{figure*}
\begin{center} 
\includegraphics[width=.97\columnwidth]{EoSs.eps}
...
\end{equation}
One can extract $Q_{ij}$ and $\varepsilon_{ij}$ -- and thus $\lambda$, or its dimensionless form $\Lambda \equiv \lambda/M^5$ -- via a double asymptotic expansion of the gravitational potential in a buffer zone, defined by ${\cal{L}} \gg r \gg R$: \ky{Added a minus sign in front of $\frac{1+g_{tt}}{2}$.}
\begin{align}
...
The calculation of the tidal deformability therefore requires the calculation of the metric component $g_{tt}$. One typically starts by first constructing a spherically symmetric, non-spinning background solution, whose stellar radius and mass are determined from $p(R)=0$ and \kent{$M=(1-g_{rr}(R)^{-1})R/2$}. One then perturbatively introduces a tidal deformation and solves the perturbed Einstein equations in the NS interior, and matches this solution to an exterior \sout{Schwarzschild} \ky{The exterior solution is not Schwarzschild.} solution at the surface, modulo \sout{a constant} \kent{two integration constants}. The requirement that the metric be at least $C^{1}$ fixes \sout{all} \kent{the ratio between the two} constants of integration, yielding the tidal deformability, as discussed in more detail in~\cite{hinderer-love}. 

\ky{I created this new paragraph. Please check.}
...
%One can similarly obtain the moment of inertia from the asymptotic behavior of the $g_{t \phi}$ metric component.
...
\ky{Somehow we've been citing~\cite{Yagi:binLove} for the C-Love relation, but I think this should really be~\cite{Maselli:2013mva} so I've modified throughout.}
...
\kent{In the top} left panel of Fig.~\ref{fig:clove}, we show the C-Love relations \kent{for hybrid stars (dashed green curves), while in the bottom panel, we show the absolute difference of the relations for such stars from the fit constructed only from} the constrained set (dashed green curves). The C-Love relations for hybrid stars remain EoS-insensitive, but the degree of universality is not as high as in the case of hadronic NSs. Table~\ref{tab:hybridCompareClove} compares the maximal EoS variability for the constrained and hybrid star EoSs fitting to the three data sets described above. As in Sec.~\ref{sec:ilq-hyb}, the maximal EoS variation for hybrid stars fluctuates only slightly ($\sim 4.5\% - 7\%$) in each case. From this, we conclude that hybrid stars do obey the C-Love relation derived for nuclear matter stars, with the caveat that the maximum universality increases to $\sim 7.1\%$.

\ky{Why don't we show R-Love for hybrid stars?}
...
For all 3 cases, the hybrid EoSs are only universal up to a minimum of $\sim1$\% (fractional difference from the fit), while in each case the constrained EoSs outperform the hybrid ones \ky{Is this true? For case 3, hybrid one has smaller number than the constrained one.}. The second case is also shown in Fig.~\ref{fig:clove}.
...
Let us now consider the binary Love relations. \kent{As for other relations, we consider nuclear matter stars and hybrid stars separately.} 
...
\end{equation}
where $\mathcal{N}(\mu_r,\sigma_r)$ is a normal distribution with mean and variance $\mu_{r}$ and $\sigma_{r}^2$. Let us further assume that the residuals $r$ obey a Gaussian distribution with mean and standard deviation that can be decomposed as \ky{Are you }
\begin{align}
...
\end{center}
\caption{
\ky{What are $\Lambda_s$ and $q$ used to produce this plot?}
...
\ky{Can we have 4 small ticks between large ticks instead of 3?}
}
...
We begin by testing our Fisher analysis (with an IMRD + 6PN waveform injection) against a simulated event identical to GW170817 with O2 detector sensitivity~\cite{aLIGO}. Because only 1 event was detected, we use $\tilde\Lambda$ and $\delta\tilde\Lambda$ as the tidal parameters for comparison purposes. Further, we scale the luminosity distance such that the signal-to-noise-ratio ($SNR \equiv \rho$) is fixed to $\rho=32.4$, as found in GW170817. \ky{I think we should also explain what fiducial values we used for all the other parameters, like masses and $\tilde \Lambda$.} We also assume low spin priors $|\chi| \leq 0.05$, as well as $\tilde{\Lambda} \leq 3000$ and $|\delta \tilde{\Lambda}| \leq 500$~\cite{Wade:LambdaPriors}. The resulting $90\%$ credible region of the posterior distribution on $\tilde{\Lambda}$ has a range of $\pm 276.99$, which is in close agreement to that found by LIGO~\cite{TheLIGOScientific:2017qsa,Abbott2018}
...
\ky{Please change ``a++'' to ``A+'' in the legend. Same comment applies to Fig.~9.}
}
...
\ky{Do we really need to show numbers with 7 digits...? I think it's enough to just show the first few digits in a form $a.b \times 10^X$ in each cell.}
}\label{tab:variances}
\end{table*}
