...
Similarly, another set of equation of state independent relations exist between individual neutron star parameters: moment of inertia (I), tidal deformability (Love number), quadrupole moment (Q), and compactness (C) - known as the ``I-Love-Q" and ``C-Love" universal relations.
...
\kc{The paper is fairly short, is the summary here necessary?}
...
Additionally, we find that hybrid quark-hadron stars do obey the I-Love-Q and C-Love universal relations, with sightly increased EoS variability.
...
Unfortunately\kc{??}, due to the current limitations in detector sensitivity, the systematic errors arising from EoS variation in the universal relations are far outweighed by the statistical errors accrued by extraction of high-order PN tidal parameters from GW waveforms.
Fig.~\ref{fig:stackedFisher} compiles the results from Fisher analyses approximating the uncertainties accrued from parameter extraction for GW170817 as if it had been observed by future detectors $A \equiv ($ O2, aLIGO, a\texttt{++}, Voyager, ET, CE$)$\zc{cite these individually}\kc{a\texttt{++} should be A\texttt{+}}.
Here, $\sigma^A_{\text{GW170817}}$ and $\rho^A_{\text{GW170817}}$ correspond to the statistical error and signal-to-noise-ratio found from observing GW170817 on detector $A$, and $\sigma^A_N$ represents the combined uncertainty after the detection of $N_A$ events (corresponding to the binary NS merger rate associated interferometer $A$)\kc{How are we combining the uncertainty on $\tilde{\Lambda}$ when each event will have a different $\tilde{\Lambda}$?}.
These values are further tabulated in Table~\ref{tab:variances}.
It can be seen from this figure that the statistical errors from parameter extraction become comparable the universal relation systematics (indicated by the magenta horizontal line) for the Voyager telescope - indicating when improved binary Love universal relations will become viable\kc{viable? you mean necessary?}.
\begin{figure}
\begin{center} 
...
We continue in Sec.~\ref{sec:universal} by finding new and improved binary Love, I-Love-Q, and C-Love universal relations, and considering how well hybrid star EoSs agree with the universal relations.
...
Given that all currently proposed EoSs utilize certain approximations\kc{cite reviews by Baym and Oertel}, one method to study a wide range of physically realizable EoSs is to parameterize them in a model-independent\kc{how is this model-independent?} way.
...
Additional constraints imposed upon the generated EoSs were as follows~\cite{LIGO:posterior}: (i) causality within 10\%, and (ii) EoS priors must support NS masses up to $1.97 \text{ M}_{\odot}$, consistent with astrophysical observations\kc{cite}.
We utilize a random set of 100 of the above posterior samples defined by GW170817, furthermore referenced as the ``constrained EoSs", as shown in Fig.~\ref{fig:eos}.\zc{Carl, can you describe how you generated these? They aren't the \emph{actual} posteriors, are they?}\kc{Yeah they are}
...
Similar universal relations have been found to exist between individual NS observables: moment of inertia (I), tidal deformability (Love), quadrupole moment (Q), and compactness (C), known as the ``I-Love-Q" and ``C-Love" universal relations~\cite{Yagi:ILQ, Yagi:binLove}.
...
While the two above fits both result in fits with similar $R^2$\kc{define} values of $\sim 0.9999995$ for the data presented, the lattter one has the advantage that it properly limits to the Newtonian case as $\Lambda \rightarrow \infty$~\cite{Yagi:binLove}.
...
\centering
\caption{
I-Love-Q and C-Love universal relations fit parameters for the constrained EoS data using the improved fitting relations found in Eq.~\ref{eq:ILQfitNew}.
...
\centering
\caption{
Comparison between the universal relations (I-Love-Q, C-Love, and binary Love) maximal EoS variability for the results of previous works~\cite{Yagi:ILQ,Yagi:binLove}, and the unconstrained and constrained sets of EoSs analyzed here. 
...
In this section we consider approximate C-Love universal relations between NS compactness $\bar{C} \equiv M/R$ and unitless tidal deformability $\Lambda$, as discussed by Yagi and Yunes~\cite{Yagi:binLove}.
...
\begin{equation}
\bar{C}^N=C_{\bar{C}\Lambda}\Lambda^{-1/5}.
\end{equation}
The new fitting coefficients are displayed in Tab.~\ref{tab:ILQfitNew}, alongside the previously found I-Love-Q relations.

Figure~\ref{fig:clove} presents the C-Love universal relations for both the constrained and unconstrained sets of EoSs, along with the relative fractional difference from the fits. 
...
\begin{figure}
\begin{center} 
\includegraphics[width=1\columnwidth]{CL.eps}
\end{center}
\caption{
Similar to Fig.~\ref{fig:ILQ} but for C-Love universal relations.
...
In this section, we consider the effect of hybrid star EoSs on the C-Love universal relation.
...
From this, we similarly conclude that hybrid stars do obey the C-Love universal relation derived with nuclear matter stars, with the caveat that the maximum universality increases to $\sim 7.1\%$.
This is exemplified in Fig.~\ref{fig:clove} by the dashed green curves.

\begin{table}
\centering
\caption{
Similar to Tab.~\ref{tab:hybridCompare} but for the C-Love universal relation.
Observe how for all 3 cases, the hybrid EoSs are only universal up to a minimum of $\sim1$\%, while in each case the constrained EoSs outperform the hybrid ones.
...
\begin{tabular}{ c  || c c } 
 ...
 \emph{(Case 1)} & &\\
 \cline{1-1}
 \emph{Constrained only} & \multirow{2}{*}{$0.0.0216879$} & \multirow{2}{*}{$0.0.0719104$}\\
  \emph{(Case 2)} & &\\
  \cline{1-1}
 \emph{Hybrid only} & \multirow{2}{*}{$0.0.0578764$} & \multirow{2}{*}{$0.0.0447799$}\\
  \emph{(Case 3)} & &\\
  \cline{1-1}
\hline
\hline
...
In addition, we find that binaries consisting of one massive hybrid quark-hadron star, and one small-mass hadron star do not obey the binary Love universal relations -- explained by the large difference in $\tilde{\Lambda}$ displayed between the constituent stars.
